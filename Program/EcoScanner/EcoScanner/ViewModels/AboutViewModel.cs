				...
				if(int.TryParse(Result.Text, out int number))
				{
					...
					else
					{
						...
						WarningPopupViewModel viewmodel = new WarningPopupViewModel("For stort tal - Bør være under 500",
							new SingleButtonView(new StandardTwoButtonViewModel(async () => await ButtonCommands.ClosePopupAsync(),
							async () => await ButtonCommands.ClosePopupAsync(), "TilbageKnap.png", "")));
